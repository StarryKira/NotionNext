...
**`themes/claudedocs/`** — Claude Docs 风格主题（新开发）。三栏布局（侧边栏 272px | 内容区 | TOC 220px），使用 Anthropic 品牌字体和陶土橙配色，支持深色模式和移动端响应式。激活方式：`.env.local` 中设置 `NEXT_PUBLIC_THEME=claudedocs`，或 URL 参数 `?theme=claudedocs`。
