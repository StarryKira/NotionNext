...
export async function fetchGlobalAllData({
  ...
  // 兜底数据绝不能进缓存：EmptyData 是个 truthy 对象，cache_manager 会把它当成
  // 正常结果写入，于是一次拉取失败就被永久固化，后续请求全部读到空站点。
  // 因此让异常穿透 getOrSetDataWithCache（失败即不写缓存），在外层兜底。
  try {
    cachedData = await getOrSetDataWithCache(cacheKey, async () => {
      ...
      if (!data) {
        throw new Error(`未能获取站点数据 pageId:${pageId}`)
      }

      return handleDataBeforeReturn(deepClone(data))
    })
  } catch (error) {
    console.error('全站数据获取失败，本次返回兜底数据且不写入缓存', error)
    cachedData = handleDataBeforeReturn(deepClone(EmptyData(pageId)))
  }
